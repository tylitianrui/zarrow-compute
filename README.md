...
- `sort_indices` / `array_sort_indices` (vector, current `int32/int64/float64` subset, configurable via `Options.sort`)
...
- `is_finite` / `is_inf` / `is_nan` (vector, current `float64` subset, `Options.none`)
...
- `equal` / `not_equal` / `less` / `less_equal` / `greater` / `greater_equal` (vector, current `int32/int64/float64` subset, `Options.none`)
- `invert` / `and_` / `or_` / `and_kleene` / `or_kleene` / `xor` / `and_not` / `and_not_kleene` (vector, `bool`, `Options.none`)
- `subtract_i64` (vector, current `int32/int64/float64` subset)
- `divide_i64` (vector, current `int32/int64/float64` subset)
- `multiply_i64` (vector, current `int32/int64/float64` subset)
...
take => [3, null, null, 1]
array_take => [3, null, null, 1]
indices_nonzero => [2, 4]
fill_null => [1, 9, 3]
fill_null_forward => [null, 2, 2, 4, 4]
fill_null_backward => [2, 2, 4, 4, null]
equal => [true, null, false]
and_kleene => [false, null, null]
or_kleene => [null, true, true]
cast_i32_to_i64 => [1, 0, 2]
cast_i32_to_bool => [true, false, true]
count => 2
sum => 4
min => 1
max => 3
